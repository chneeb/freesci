			       ...
			       int flags, int default_palette, int nr, void *internal)
{
	...
	if (state->version >= SCI_VERSION_01_VGA) {
		...
		{	/* Push visual to PSRAM; pico_blit_indexed handles psram_valid==1.
			...
			/* The control pass borrows this buffer as its flood-fill aux_map.
			   That works PACKED too: on the unscaled path the aux needs at most
			   four bits (clipmask + CLIPMASK_HARD_BOUND), so a nibble-packed
			   visual[0] is big enough to be the aux -- see the accessors in
			   sci_picfill_aux.c. This is what keeps 4bpp a real saving: no
			   separate 64KB transient, exactly as before packing. */
