...
/* scidisasm_safe: a hardened variant of scidisasm that NEVER writes into the
 * game directory. Output always goes to a separate directory.
 *   -d <gamedir>            read-only resource dir (unchanged from scidisasm)
 *   env DISASM_OUT=<dir>    output dir (default /tmp/sq3_disasm), auto-created
 *   env DISASM_SCRIPT=<n>   if set, dump only script n; else dump all scripts
 * Build (from repo root, after a desktop CMake build exists):
 ...
 *       -o /tmp/scidisasm_safe src/tools/scidisasm_safe.c \
 *       -Wl,--start-group $LIBS -Wl,--end-group -lSDL2 -lm -lz -lpthread -ldl
 * Run:
 *   DISASM_SCRIPT=10 /tmp/scidisasm_safe -d ~/Downloads/sq3
 *   cat /tmp/sq3_disasm/010.script
 */
...
int main(int argc, char** argv)
{
	...
	{
		const char *outdir = getenv("DISASM_OUT");
		int only = getenv("DISASM_SCRIPT") ? atoi(getenv("DISASM_SCRIPT")) : -1;
		if (!outdir) outdir = "/tmp/sq3_disasm";
		mkdir(outdir, 0777); /* output dir, never the game dir */
		...
		printf ("Performing second pass -> %s ...\n", outdir);
		for (i=0; i < resmgr->resources_nr; i++)
			if (resmgr->resources[i].type == sci_script)
				{
					int num = resmgr->resources[i].number;
					if (only >= 0 && num != only)
						continue;
					snprintf (outfilename, sizeof(outfilename),
						  "%s/%03d.script", outdir, num);
					open_console_file (outfilename);
					disassemble_script(&disasm_state, num, 2);
					close_console_file();
				}
	}
